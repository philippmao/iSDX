...
Every participant has its own Swift-BPA process running \rb{is this really true. I thought Swift-BPA only runs once for an entire SDX and sits in central services}. The Swift-BPA only receives BGP updates from his own routers. This way the Swift engine can be implemented without any modifications.
...
The encoded AS-path starts with the second AS on the AS-path. This is because the first AS is already encoded as the BGP best next hop. (in the iSDX part of the VMAC) \rb{weird comment does it belong to the previous sentence if so change punctuation}
